...
const DictionaryTable = ({
  ...
  const save = async (id) => {
    try {
      ...
      let updateWordData = await updateWord({
        lid: queryParam,
        data: newData[index],
      });
  ...
  useEffect(() => {
    ...
    for (const colHeader in Object.keys(firstObject)) {
      if (
        Object.keys(firstObject)[colHeader] !== "id" &&
        Object.keys(firstObject)[colHeader] !== "key"
      ) {
        // add to remove id and key columns -> for testing purposes only
        ...
        const col = {
          ...
          sorter: (a, b) => {
            console.log(
              typeof a[Object.keys(firstObject)[colHeader]],
              typeof b[Object.keys(firstObject)[colHeader]]
            );
            if (
              typeof a[Object.keys(firstObject)[colHeader]] === "number" &&
              typeof b[Object.keys(firstObject)[colHeader]] === "number"
            ) {
              return (
                a[Object.keys(firstObject)[colHeader]] -
                b[Object.keys(firstObject)[colHeader]]
              );
            } else {
              return String(
                a[Object.keys(firstObject)[colHeader]]
              ).localeCompare(String(b[Object.keys(firstObject)[colHeader]]));
            }
          },
          sortDirections: ["descend", "ascend"],
          ...getColumnSearchProps(Object.keys(firstObject)[colHeader]),
          onFilter: (value, record) => {
            return String(record[Object.keys(firstObject)[colHeader]]).includes(
              value
            );
          },
        };
        ...
        return editable ? (
          ...
          <div style={{ minWidth: "200%" }}>
            ...
            <Popconfirm
              ...
              onConfirm={async () => await handleDelete(record.id, record)}
            >
              <a>
                <DeleteOutlined style={{ marginLeft: 12 }} />{" "}
              </a>
            </Popconfirm>
          </div>
        );
    ...
    const unregisterFunction = onValue(dictRef, (snapshot) => {
      ...
      if (Object.keys(firstCol).length === 1) {
        ...
        for (const colHeader in Object.keys(result)) {
          if (
            Object.keys(result)[colHeader] !== "id" &&
            Object.keys(result)[colHeader] !== "key"
          ) {
            // add to remove id and key columns -> for testing purposes only
